...
// Protocoles
void ACCURACYandLOSS(MLP_Network_ADAM *mlp,float **dataSet, int nDataSet,char **desiredOutputs, float *accuracy,float *loss);
void signal_callback_handler(int signum);


int main(int argc, char *argv[])
{
    cout << "SGD Train_Good_Evening " << endl;
...
/////////////////////////// Load et Allocation Training set////////////////////////////////////
...
////////////////////////// Load et Allocation Testing set  ////////////////////////////////////
    cout << " \n \n" ;
...
// mlp network allocation
    MLP_Network_ADAM mlp(0.8,0.9,'L');
    mlp.Allocate(nInputUnit,nHiddenUnit,nOutputUnit,nTrainingSet);

// Prepare rawname for intermediate saving
    string fullname = SRC_PATH+"/models/"+argv[1];                          // with the line command, fullname = C:/Users/Zhenchen Zhang/Desktop/cppV2/models/sgd/irm_sgd.bin
...
// LOG and GRAPH
    ...
    fstream adam_log; // variable of type "file"
...
// THREAD FOR REAL TIME UPDATE
    ...
    GenerateGraphAcc(adam_log_filename, gnuplot_graph_acc, maxEpoch, true); // write relative configurations parametres in the file whose name is gnuplot_graph_acc
    string system_call_string(GNUPLOT_PATH+" "+gnuplot_graph_acc);  // create an object "system_call_string" of string preparing for launching the prog. gnuplot
    std::thread gnuplot_thread_acc( system, system_call_string.c_str() ); // create an object "gnuplot_thread_acc" of std::thread 创建一个线程对象
//                                                                           lauch the program gnuplot and create les courbes
...
//                ProgressBar('P', 1.0*i / nTrainingSet, string(" - " + std::to_string(local_time)).c_str(), 60); // Print a jinDuTiao in the terminal
            ...
            // update after the whole batch is treated (when all images of the batch go through the 2 passes)
        ...
        ProgressBar('C'); // clear the progress bar
...
// SAVE IF CURRENT IS THE BEST
        if(AccuracyTest > maxTestAccuracyRate)
            ...
            std::ofstream os(best_filename, std::ofstream::binary); // output stream
            os<< mlp; // save the model
            os.close();
            cout << " * " << endl;
            }
        else cout << endl;
